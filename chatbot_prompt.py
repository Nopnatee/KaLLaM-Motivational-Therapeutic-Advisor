...
if __name__ == "__main__":
    try:
        ...
        # Example conversation with proper message format
        chat_history = []
        ...
        print(f"KaLLaM Response 1: {response}")
        
        # Add to chat history
        chat_history.extend([
            {"role": "user", "content": "สวัสดีค่ะ ฉันมีปัญหาเรื่องนอนไม่หลับ"},
            {"role": "assistant", "content": response}
        ])
        
        # Second message
        response2 = chatbot.get_chatbot_response(
            chat_history=chat_history,
            user_message="นอนไม่หลับมา 3 วันแล้วค่ะ กังวลเรื่องงาน",
            health_status="มีปัญหานอนไม่หลับ, ความดันโลหิตสูง, stress จากงาน",
            summarized_histories=None
        )
        print(f"KaLLaM Response 2: {response2}")
